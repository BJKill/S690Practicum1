...
```{r}
ggplot(p1.Data, aes(x=Total_Met_Min, y=..density..))+geom_histogram()
ggplot(p1.Data, aes(x=log(Total_Met_Min+1), y=..density..))+geom_histogram()+geom_density()
```
...
```{r}
ggplot(p1.Data, aes(x=shift, y=lbs_gained)) +geom_boxplot()
anova(lm(lbs_gained~shift, data= p1.Data))
...
## ZIP Model
```{r Model Estimation, warning=T}
zipmod.ord <- zeroinfl(round(lbs_gained,0)~oshift:log(Total_Met_Min+1)+oshift:beg_weight+oshift*gender+log(Total_Met_Min+1)*gender+beg_weight*gender+Age, data = na.omit(p1.Data))
summary(zipmod.ord)
...
```{r Diagnostic Plots}
plot(log(fitted(zipmod.ord)), log((na.omit(p1.Data)$lbs_gained-fitted(zipmod.ord))^2),
     xlab=expression(hat(mu)[i]),
     ylab=expression((y[i]-hat(mu)[i])^2),
     main = "Estimated Variance vs. Estimated Mean")
abline(0,1)

halfnorm(residuals(zipmod.ord), main = "Half-Normal QQ Plot of Residuals")

plot(fitted(zipmod.ord), resid(zipmod.ord),
     ylab="Residuals",
     xlab="Fitted Values",
     main = "Residual Plot")
```

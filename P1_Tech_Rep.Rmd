...
```{r}
ggplot(p1.Data, aes(x=Age, y=lbs_gained, color=gender)) +geom_point()
```

```{r}
ggplot(p1.Data, aes(x=shift, y=lbs_gained, color=gender)) +geom_point()
```

```{r}
ggplot(p1.Data, aes(x=beg_weight, y=lbs_gained, color=gender)) +geom_point()
```

## ZIP Model
```{r Model Estimation}
zipmod <- zeroinfl(round(lbs_gained,0)~oshift+I(Total_Met_Min/100)+beg_weight*gender+Age*gender, data = na.omit(p1.Data))
summary(zipmod)
```

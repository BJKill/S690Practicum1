...
\item The overall effect is \alert{negative}.\\
\item This negative effect is \alert{weaker} with later shifts.
...
\item The overall effect is \alert{negative}.\\
\item This negative effect is \alert{stronger} in men.
...
\begin{itemize}
\item Treated as a count of pounds gained over 8 months.
\item We distinguish between those who gained weight and those who did not (0 lbs gained).
\end{itemize}
\end{alertblock}

```{r, echo=F, warning = F, message = F, fig.height= 2, fig.width=4, fig.cap= "We can think of Pounds gained as a Poisson random variable with an inflated zero count."}
...
\item weighted sum of walk, moderate, and vigorous exercise.
\end{itemize}
\end{block}
...
\begin{itemize}
\item 7 observations of weight gain fall outside of the 95% IQR.
\item Model fit is improved with the omission of these observations.
\begin{enumerate}
...
### Interactions
- We do not have a theoretical expectation for interaction effects.
- We check for interactions with gender and shift.
- Inclusion was based on examination of different slopes for different factor levels.
...
We only identified TMM as requiring a transformation (natural log) to compress the scale and reduce the right-skew.
...
- Binomial model for gaining weight or not:
    ...
    + We found no significant predictors in this model.

- Poisson Model for number of pounds gained:
    + Coefficients represent change in log-mean.
    + We found 6 significant predictors.
:::
...
:::{.block}
### Main Effects
- TMM has a \alert{negative }effect on gained weight.
...
\begin{itemize}
\item We need more research (preferably RCT) on who gains weight.
\item Causal inference is based on strong assumptions and should be tested farther.
\item This study does not distinguish between healthy and unhealthy weight gain.
\end{itemize}
\end{alertblock}

...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Only two outliers remain after removing the first eight. However, these would not be outliers if they fell into the category shift = `other`, enticing us to leave them in."}
p1.Data2 %>%
  group_by(shift) %>%
  mutate(outlier = ifelse(is_outlier(lbs_gained), lbs_gained, as.numeric(NA))) %>%
  ggplot(., aes(x = shift, y = lbs_gained, color = gender)) +
  ...
  scale_color_manual(values  =  cb_palette[c(2,3)]) +
  labs(title = "Side-by-Side Boxplot of Pounds Gained vs. Shift split by Gender",
       subtitle = "Initial outliers removed. New outliers labeled by their Pounds Gained",
       x= "Shift",
       y= "Weight Gain" )
```
